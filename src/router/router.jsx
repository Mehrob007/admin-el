...
import RefreshToken from "../components/componentsLayout/RefreshToken";
import Sizes from "../components/componentsLayout/Sizes";
...
const suspense = (htmlElem) => (
  <Suspense fallback={<h1>Загрузка...</h1>}>{htmlElem}</Suspense>
);
  ...
  return (
    <Routes>
      <Route path="/" element={suspense(<Layout />)}>
        ...
        <Route path="sizes" element={suspense(<Sizes />)} />
        <Route path="token/:token" element={suspense(<RefreshToken />)} />
      </Route>
